...
const Statistics = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCounts(prev => ({
        scholarships: Math.min(prev.scholarships + Math.ceil(finalCounts.scholarships / steps), finalCounts.scholarships),
        students: Math.min(prev.students + Math.ceil(finalCounts.students / steps), finalCounts.students),
        countries: Math.min(prev.countries + Math.ceil(finalCounts.countries / steps), finalCounts.countries),
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          {stats.map((stat, index) => (
            ...
            >
              <motion.div
                ...
                className={`text-5xl md:text-6xl font-extrabold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent mb-4`}
              >
                {stat.number}
              </motion.div>
        ...
        >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="inline-block px-8 py-4 rounded-xl font-semibold bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 text-white shadow-lg hover:shadow-2xl transition-all duration-300"
          >
            Get Started Now
          </motion.a>
        </motion.div>
